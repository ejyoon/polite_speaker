...
The relationship between negation and politeness is a topic of long-standing interest to linguists and psychologists [@stoffel190-FIXME; @stern1937-FIXME; @bolinger2013; @horn1989].
...
\(\omega\), that determines how much Ann prioritizes each of the three possible goals, as well as a goal weight to convey to the listener \(\phi_{S_1}\); more details below).
Following standard practice in RSA models, Ann's production decision is softmax, which interpolates between choosing the maximum-utility utterance and probability matching [via speaker optimality parameter $\lambda_{S_2}$; @goodman2013]:

\begin{equation}
P_{S_2}(w | s, \omega) \propto \exp(\lambda_{S_2} \cdot \mathop{\mathbb{E}}[U_{total}(w; s; \omega; \phi_{S_1})]). \label{eq:S2}
\end{equation}


We posit that a speaker's utility contains distinct components that represent three possible goals that speakers may entertain: informational, social, and presentational. These components were determined based on multiple iterations of preliminary experiments, after which we conducted the preregistered test of our specified model with these specific utilities that we report below.
...
\begin{equation}
U_{total}(w; s; \hat{\phi}; \phi_{S_1}) = \phi_{inf} \cdot U_{inf}(w; s) + \phi_{soc} \cdot U_{soc}(w) + \phi_{pres} \cdot U_{pres}(w; \phi_{S_1}) - C(w). \label{eq:UTotal}
\end{equation}
...
Next, we define *social utility* (\(U_{soc}\)) to be the expected subjective utility of the state \(V(s)\) implied to the
...
\begin{equation}
P_{L_1}(s, \phi_{S_1} | w) \propto P_{S_1}(w | s, \phi_{S_1}) \cdot P(s) \cdot P(\phi_{S_1}). \label{eq:L1}
\end{equation}
...
Only the social and informational utilities are defined for the first-order \(S_1\) speaker (via reasoning about \(L_0\)); thus, \(S_1\)'s utility weightings can be represented by a single number, the mixture parameter \(\phi_{S_1}\).
Definitions for \(S_1\) and \(L_0\) otherwise mirror those of \(S_2\) and \(L_1\) and we use the same speaker optimality parameter for S_1 as for S_2 for simplicity; these model definitions and predictions by these different listener and speaker models can be found in the Model Predictions section.
...
The speaker then chooses utterances $w$ softmax-optimally given the state s and his goal weight mixture \(\phi_{S_1}\).

The pragmatic listener model $L_1$ reasons jointly about both the true state of the world and the speaker’s goals (Fig. \ref{fig:schematicPredictionsFig}, middle). Upon hearing [You poem was] “amazing”, the listener faces a tough credit-assignment problem: The poem could indeed be worthy of three hearts, but it is also possible that the speaker had strong social goals and then no inference about the quality of the poem is warranted. Hearing [Your poem] was “terrible”, the inference is much easier: the poem is probably truly terrible (i.e., worthy of zero hearts) and the speaker probably does not have social goals. Negation makes the interpreted meanings less precise and hence, inferences about goals are also fuzzier: “not amazing” can be seen as a kind way of saying that the poem was worthy of 0 or 1 hearts, which satisfies some amount of both social and informational goals. “Not bad” is less clear: the speaker could be being nice and the poem was actually worthy of 0- or 1-hearts (i.e., it was bad) or the speaker could be being honest (i.e., it was not bad) and the poem was worth 2-hearts. 
...
The interesting novel behavior of this level of recursion comes from the different flavors of the self-presentational goal. 
When the second-order pragmatic speaker wants to *appear* to be social (i.e., present themselves as someone who is kind) they even more strongly display the preference for utterances that signal positive states (i.e., they are over-the-top positive). .
When the speaker wants to appear honest and informative, they take the exact opposite strategy, producing utterances that cannot be explained by virtue of social utility: direct, negative utterances (e.g., “it was terrible”).
...
```{r screenshot,  out.width = "70%", fig.pos = "!h", fig.cap="Example of a trial in the speaker production task."}
...
We use uninformative priors over ranges consistent with the prior literature on RSA models: $\theta_{s,w}^{lit} \sim \text{Uniform}(0, 1)$, $phi_g \sim \text{Uniform}(0, 1)$, $\omega_g \sim \text{Dirichlet}(1,1,1)$, $\alpha \sim \text{Uniform}(0, 20)$, $c \sim \text{Uniform}(1, 10)$.
...
We compared these to six model variants containing subsets of the three utilities in the full model using Bayes Factors.
...
That is, simply because a model has more parameters and can explain more of the variance in the data set does not entail that it will have the highest marginal likelihood of the data.
That is not the case here, however: both the variance explained and marginal likelihood of the observed data were the highest for the full model: The full model was at least 5 x 10^4 times better at explaining the data than the next best model (Table \@ref(tab:comparisonTable)). 
...
In sum, the full set of informational, social, and presentational were required to fully explain participants' utterance choices.
...
*Being informative* ("informative") pushes the weight on social utility ($\phi_{soc}$) close to zero, but the weight on *appearing kind* ($\phi_{pres}$) stays high, suggesting that speakers are expected to manage their own face even when they are not considering others'.
...
Each utterance will have strengths and weaknesses relative to the speaker's goals. Although computation in an unbounded model presents technical challenges [perhaps paralleling the difficulty human speakers feel in finding the right thing to say in a difficult situation; see @goodman2016], addressing these challenges is an important future direction.
...
Our work shows how social and self-presentational motives are integrated with informational concerns more generally, opening up the possibility for a broader theory of social
language.
In addition, a formal account of politeness moves us closer to courteous computation -- to machines that can talk with tact.
