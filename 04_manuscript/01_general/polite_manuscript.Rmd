...
$$P_{L_0}(s | w) \propto  \mathcal{L}(u, s) \cdot P(s).$$

\noindent where $\mathcal{L}(u, s) = \theta \in [0,1]$ and $\theta$ is the probability that the utterance $u$ is true of state $s$, which is a generalization of more traditional, binary truth functional semantics (see @degen2020 for a discussion of this kind of “soft semantics”).
The literal meaning is used to update the literal listener's prior beliefs
over world states $P(s)$.
$\theta$ is estimated empirically from the Literal Semantic task described in the next section, where it is roughly the proportion of participants who endorse the utterance $u$ in state $s$ in the Literal Semantic task.
