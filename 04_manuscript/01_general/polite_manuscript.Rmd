...
\usepackage{color}
\newcommand{\mht}[1]{\textcolor{green}{[mht: #1]}}
...
This language is typically inefficient and underinformative, and sometimes even outright false. 
...
a social utility, derived by being kind and providing positive affect to others; \mht{is there a way of describing the social utility and presentational in terms of "face"?}
...
Epistemic, social, and self-presentational utilities are weighed within a Rational Speech Act (RSA) model. 
...
If listeners try to infer the goals that a speaker is entertaining (e.g., social vs. epistemic), speakers may choose utterances in order to convey that they had certain goals in mind. 
...
In our work, utterances with negation (e.g., “not terrible”) are slightly more costly than their equivalents with no negation (given by a parameter inferred from data; see Supplemental Materials). 

Intuitively, if Bob’s performance was good, Ann’s utilities align to lead her to say something positive. 
By saying “[Your poem/recital] was amazing,” Ann is simultaneously being truthful, kind, and appearing both and truthful and kind.
If Bob’s performance was poor, however, Ann is in a bind: 
Ann could be kind and say "It was great", but she does so at the cost of conveying the wrong information to Bob (e.g., if he mistakenly infers Ann’s goal to be truthful and learns that his recital was actually good).
...
It conveys some true information (e.g., literally it was not the absolute worst) while being sufficiently open-ended to spare Bob’s feelings. 
...
Using an independent sample of *N*=51 participants, we measured judgments of how well different utterances apply to each of the levels on the heart scale (e.g., to what extent is “terrible” true of 2 out of 3 hearts?). 
These measurements are used in the Bayesian data analysis to approximate the semantics of the words as interpreted by the literal listener agent $L_0$ (see Supplementary Materials for literal semantic results; see our pre-registered model, hypothesis, and procedure at FIXME).
...
The full model was superior to: the model with social and presentational utilities, which predicted outright false statements (“It was good”); the model with informational and social utilities, which predicted truthful statements “It was terrible” and “It wasn’t amazing” (that is semantically true when the poem was terrible); and to the model with informative and presentational utilities, which predicted that the speaker would now care about being seen as informative and nice, but still wanting to be as truthful (“It was terrible”) as she is presentational (“It wasn’t terrible”).
Thus, all three utilities -- informational, social, and presentational -- were required to fully explain participants’ choices, correctly predicting that they would prefer to say an indirect speech (“It wasn’t terrible”) about a bad performance.

Politeness is a puzzle for purely informational accounts of language use. 
Incorporating social motivations can provide an explanatory framework, but such intuitions have been resistant to formalization or precise testing. 
To overcome this issue, we created a utility-theoretic model of language use that captured the interplay between competing informational, social, and presentational goals. 
A preregistered experimental test of the model confirmed its ability to capture human judgments, unlike comparison models that used only a subset of the full utility structure. 
...
Brown and Levinson, and other theories of politeness describe the other- vs. self-oriented strategies as qualitatively different [e.g., maximize approval of other, minimize praise of self; @leech1983], but did not explain how the motivations of the two are related or how they trade off to inform the speaker’s utterance choices. 
