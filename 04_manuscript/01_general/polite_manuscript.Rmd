...
\(\omega\), that determines how much Ann prioritizes each of the three possible goals, as well as the goal weight she wishes to convey to the listener \(\phi_{S_1}\); more details below).
...
Definitions for \(S_1\) and \(L_0\) otherwise mirror those of \(S_2\) and \(L_1\) and we use the same speaker optimality parameter for S_1 as for S_2 for simplicity; predictions by these different listener and speaker models can be found in the Model Predictions section.
...
utility structure. Culture could also affect
