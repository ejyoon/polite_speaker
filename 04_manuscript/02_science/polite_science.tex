...
\(V(s)\), or expected subjective utility, of the state implied to the listener by the utterance:
...
\noindent In order to define this term, the speaker must consider that the listener \(L_1\) reasons about the speaker's utilities together with the true state of the world:
...
We built a Bayesian statistical model \cite{lee2014} to infer the parameters of
the RSA model (e.g., the speaker's utility weights in each goal condition;
...
\emph{Being nice} requires equal weights on all three utilities, indicating that Gricean informativity needs to be part of language use even when that is explicitly not the goal. 
\emph{Being informative} pushes the weight on kindness close to zero, but the weight on \emph{appearing kind} stays high, indicating that speakers are expected to manage their own face even when they are not considering other's. 
Speakers who are \emph{nice and informative} emphasize honesty slightly more than kindness. 
In all cases the presentational utilities actually have greatest weight, indicating perhaps that appearing honest and kind is more important than being so!
