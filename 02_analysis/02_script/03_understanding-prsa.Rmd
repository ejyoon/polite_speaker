...
# load(here("02_analysis/01_data/s2_schematic_predictions.RData"))
...
```{r}
fig.marginal.self_pres.s2 <- rs.s2 %>%
  # filter(state == 0) %>%
  # filter(state == 0, phi_S1 %in% c(0.1, 0.3, 0.5, 0.7, 0.9)) %>%
  ...
  ylab("Self-presentational speaker\nproduction probability")
...
fig.marginal.utility.s2 <- rs.s2 %>%
  ...
  scale_y_continuous(limits = c(0, 0.75), breaks = c(0, 0.75), position = "left")+
  # ggtitle("Self-presentational mixture (0 = fully social; 1 = fully informational)")+
  ylab(expression("Schematic " * S[2] * "'s \nproduction probability"))
fig.marginal.utility.s2
ggsave("../../04_manuscript/01_general/fig/s2_schematic_production.png", width = 8, height = 3)
```
...
rs.s2.tidy2 <- rs.s2.tidy %>%
                   ...
                   mutate(goal = fct_relevel(goal, "social", "informational", "presentational \n(social S1)", "presentational \n(both-goal S1)", "presentational \n(informative S1)")) %>%
                   filter(!is.na(goal))
                   # filter(phi_informative_bin == 0.9)

fig.s2 <- ggplot(rs.s2.tidy2, 
                 aes ( x = utt, y = state, fill = prob))+
  geom_tile(colour = "grey95") + 
  # scale_fill_viridis(option="plasma") +
  scale_fill_gradient(low = "white", high = "orange")+
  facet_grid(goal ~ positive, switch="y", scales = "free_x")+
  # scale_y_continuous(limits = c(0, 1), breaks = c(0, 0.5, 1))+
  guides(fill = F)+
  ylab("true state") +
...
fig.s2
ggsave("../../04_manuscript/01_general/fig/s2_utility_schematic_uttBottom.png", width = 6, height = 8)
```
...
fig.s2
ggsave("../../04_manuscript/01_general/fig/s2_utility_schematic_stateBottom.png", width = 8, height = 4)
```
...
ggsave("../../04_manuscript/01_general/fig/L1_S2_schematic.png", width = 9, height = 5)
```
